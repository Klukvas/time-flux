...
export class AuthController {
  ...
  @ApiOperation({
    ...
    description:
      'Redirects the user to Google consent screen. After authorization, Google redirects back to /api/v1/auth/google/callback.',
  })
  @ApiResponse({
    status: 302,
  ...
  async googleCallback(@Req() req: any, @Res() res: Response): Promise<void> {
    ...
    if (!profile) {
      res.redirect(
        `${this.frontendUrl}/auth/google/callback?error=GOOGLE_AUTH_FAILED`,
      );
      return;
    }
    try {
      const result = await this.authService.googleLogin(profile);
      const code = this.authService.storeOAuthCode(result);
      res.redirect(`${this.frontendUrl}/auth/google/callback?code=${code}`);
    } catch (error) {
