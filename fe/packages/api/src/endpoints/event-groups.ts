...
import type { EventGroupQueryParams } from '../types-local';

export function createEventGroupsApi(client: AxiosInstance) {
  return {
    list: (params?: EventGroupQueryParams) =>
      client
        .get<EventGroup[]>('/api/v1/event-groups', { params })
        .then((r) => r.data),
    ...
    update: (id: string, data: UpdateEventGroupRequest) =>
      client
        .patch<EventGroup>(`/api/v1/event-groups/${id}`, data)
        .then((r) => r.data),
    ...
    getDetails: (id: string) =>
      client
        .get<EventGroupDetails>(`/api/v1/event-groups/${id}/details`)
        .then((r) => r.data),

    createPeriod: (groupId: string, data: CreateEventPeriodRequest) =>
      client
        .post<EventGroup>(`/api/v1/event-groups/${groupId}/periods`, data)
        .then((r) => r.data),

    updatePeriod: (periodId: string, data: UpdateEventPeriodRequest) =>
      client
        .patch<EventGroup>(`/api/v1/periods/${periodId}`, data)
        .then((r) => r.data),

    closePeriod: (periodId: string, data: CloseEventPeriodRequest) =>
      client
        .post<EventGroup>(`/api/v1/periods/${periodId}/close`, data)
        .then((r) => r.data),
