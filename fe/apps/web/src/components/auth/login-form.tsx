...
export function LoginForm({ open, onClose, onSwitchToRegister }: LoginFormProps) {
    ...
    login.mutate(
      { email, password },
      {
        ...
        onError: (err) => {
          ...
          } else if (apiError.error_code === 'UNAUTHORIZED') {
            setFormError(t('errors.wrong_credentials'));
            toast.error(t('errors.wrong_credentials'));
          } else {
            setFormError(message);
            toast.error(message);
          }
